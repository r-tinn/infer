...
namespace Microsoft.ML.Probabilistic.Distributions.Automata
{
        ...
        where TThis : Automaton<TSequence, TElement, TElementDistribution, TSequenceManipulator, TThis>, new()
    {
        ...
        /// </summary>
        public StateCollection States => this.stateCollection;
        ...
        /// </returns>
        [Construction("GetQuotedAutomaton")]
        public static TThis FromQuoted(QuotedAutomaton data)
        {
            var result = new TThis { startStateIndex = data.StartStateIndex};
            result.stateCollection = new StateCollection(result, data.States, data.Transitions);
            result.stateCollection.CheckConsistency(result.startStateIndex);
            return result;
        }
        ...
        protected Automaton(SerializationInfo info, StreamingContext context)
        {
            this.stateCollection = new StateCollection(
                this,
                (StateData[])info.GetValue(nameof(StateCollection.states), typeof(StateData[])),
                (Transition[])info.GetValue(nameof(StateCollection.transitions), typeof(Transition[])));
        ...
        {
            info.AddValue(nameof(this.stateCollection.states), this.stateCollection.states.CloneArray());
            info.AddValue(nameof(this.stateCollection.transitions), this.stateCollection.transitions.CloneArray());
        ...
        public class QuotedAutomaton
        {
            public int StartStateIndex { get; }
            public ReadOnlyArray<StateData> States { get; }
            public ReadOnlyArray<Transition> Transitions { get; }

            [Construction("StartStateIndex", "States", "Transitions")]
            public QuotedAutomaton(
                int startStateIndex,
        ...
        public QuotedAutomaton GetQuotedAutomaton() =>
            new QuotedAutomaton(
                this.startStateIndex,
                this.stateCollection.states,
                this.stateCollection.transitions);
